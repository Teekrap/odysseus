...
async fn probe(url: &str) -> Result<u16, String> {
    ...
    match client.get(&probe_url).send().await {
        ...
        Err(e) => {
            if e.is_timeout() {
                Err("Server did not respond within 8 seconds. Check the URL and that Odysseus is reachable from this machine.".into())
...
fn open_settings(app: &tauri::AppHandle) {
    if app.get_webview_window(SETTINGS_WIN).is_some() {
        let _ = app.get_webview_window(SETTINGS_WIN).map(|w| w.show());
        return;
    }
...
fn open_server(app: &tauri::AppHandle, url: &str) {
        ...
        .build()
    {
        ...
        Err(e) => eprintln!("Failed to open server window: {}", e),
    }
}
